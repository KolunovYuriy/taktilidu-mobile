export const imgLogo = require('./logo.png')
export const icEmail = require('./mail.png')
export const icLock = require('./locked.png')
export const icShare = require('./share.png')
export const icHide = require('./hide.png')
export const icNext = require('./next.png')
export const icInstagram = require('./inst.png')
export const icVkontakte = require('./vk.png')
export const icFacebook = require('./fb.png')
export const icUser = require('./user.png')
export const icPhone = require('./phone.png')
export const icAttachment = require('./attachment.png')
export const icInfo = require('./info.png')
export const icLeftArrow = require('./left-arrow.png')
export const icRightArrow = require('./right-arrow.png')
export const icRightThinArrow = require('./rightThinArrow.png')

export const icUpArrow = require('./up-arrow.png')
export const icDownArrow = require('./down-arrow.png')
export const icDownThinArrow = require('./down-thin-arrow.png')
export const icArrowDown = require('./down-thinArrow.png')

export const icHome = require('./home.png')
export const icPlus = require('./plus.png')
export const icAlert = require('./alert.png')
export const icTicket = require('./group.png')
export const icSearch = require('./search.png')
export const icUserCircle = require('./userCircle.png')
export const icCar = require('./car.png')
export const icQA = require('./qa.png')
export const icCarStar = require('./car-star.png')
export const icMinus = require('./minus.png')

export const icEdit = require('./edit.png')
export const icFrom = require('./from.png')
export const icTo = require('./to.png')
export const icTime = require('./time.png')
export const icUsers = require('./users.png')
export const icBorder = require('./border.png')
export const icLine = require('./line.png')
export const icLine1x = require('./line-1x.png')
export const icLine2x = require('./line-2x.png')
export const icNotification = require('./notification.png')
export const icUnreadNotification = require('./unread-notification.png')
export const icSettings = require('./settings.png')
export const icBirthday = require('./birthday-cake.png')
export const icGender = require('./gender.png')
export const icCoins = require('./coins.png')
export const icLocation = require('./location.png')
export const icCirclePlus = require('./circle-plus.png')
export const icCheck = require('./check.png')
export const icMore = require('./more.png')
export const icX = require('./x.png')

export const imgSpprtBG = require('./spprt-bg.jpeg')
export const imgBG = require('./bg.png')
export const imgBgContent = require('./bg-content.jpeg')
export const imgSlider = require('./slider-image.png')

export const imgUserPhoto = require('./user-photo.png')
export const icPassenger = require('./passenger.png')
export const icPassengerEmpty = require('./passenger-empty.png')
export const icPassengerGray = require('./passenger-gray.png')
export const icCircle = require('./circle.png')
export const icStar = require('./star.png')

export const glImg1 = require('./image-1.png')
export const glImg2 = require('./image-2.png')

// Socials
export const icTelegram = require('./telegram.png')
export const icViber = require('./viber.png')
export const icWhatsapp = require('./whatsapp.png')

export const icHighFive = require('./high-five.png')
export const icCommunity = require('./community.png')
export const icCheckout = require('./checkout.png')
export const icCarFront = require('./car-front.png')
